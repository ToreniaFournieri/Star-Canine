...
hull: 200,
max_slots: 6,
inventory: [mkItem(‘🚀 Lance’), mkItem(‘🚀 Lance’), mkItem(‘⚡ Claw’), mkItem(‘⚡ Claw’), mkItem(‘🛡️ Plating’)],
equipped: []
});
...
for (let turn = 0; turn < 6; turn++) {
  ...
  if (pDmg > 0) {
    ...
    // Check for LIFE-STEAL before applying damage
    ...
    // LIFE-STEAL: If enemy shield is 0 after damage and there are LIFE-STEAL items
    if (lifeStealItems.length > 0 && eS === 0) {
      let totalHeal = 0;
      ...
      if (totalHeal > 0) {
        ...
        l.push(`  LIFE-STEAL: +${totalHeal}HP → ${beforeHeal}HP → ${pH}HP`);
      }
    }
  } else {
    l.push(`  No ${r}`);
...
<div>Hull: {Math.round(temp.hull)}/{temp.max_hull}</div>
...
return (
<div className="max-w-4xl mx-auto">
<h2 className="text-2xl mb-4 text-center">{cleared ? ‘🎉 MISSION COMPLETE 🎉’ : ‘SIGNAL LOST’}</h2>
{cleared && <pre className="text-xs mb-4 whitespace-pre-wrap text-center">{`K9 orbit cleared.
LAIKA rescued.

“You came back.”`}</pre>}
{!cleared && <div className="mb-4 text-center">Stage: {stage}</div>}

```
  <div className="mb-6 text-sm border border-green-700 p-4">
    <div className="font-bold mb-2 text-center">STAR CANINE - FINAL STATUS</div>
    <div className="mb-2">Hull: {Math.round(player.hull)}/{player.max_hull}</div>
    <div className="mb-3">Slots: {player.max_slots}</div>
    
    <div className="font-bold mb-2">INVENTORY ({player.inventory.length} items):</div>
